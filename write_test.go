...
func TestClient_WriteAndQuery(t *testing.T) {
	...
	err = testClient.Write(testDB, testRP.Name, []*Point{point}, true)
	if err != nil {
		t.Fatal(err)
		return
	}
	...
	q := testClient.NewQuery().From(testDB, testRP.Name, "measurement-2").Where(cond).Desc("time").Limit(1)
	t.Log(q.String())
...
func TestClient_WriteReliability(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		go func() {
			...
			_ = testClient.Write(testDB, testRP.Name, points, false)
		}()
	}
	wg.Wait()
	testClient.Close()
